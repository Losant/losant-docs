...
The MQTT Node allows a workflow to publish a message to the configured topic on either the Losant MQTT broker. If this is a [cloud](/workflows/cloud-workflows/) workflow, the node can also be configured to publish to one of your [MQTT broker integrations](/applications/integrations/#mqtt).
