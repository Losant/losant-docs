...
that will be rendered for each individual point in each series displayed on the map. The string template must resolve to a single URL pointing to an image and will be rendered using it's natural width and height. If the template does not resolve to an image, no marker will be displayed for that point. For SVG images, a width and/or height must be explicitly assigned in the outermost `<svg>` element in order to render.
